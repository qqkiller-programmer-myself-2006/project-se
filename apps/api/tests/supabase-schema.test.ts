...
describe("db/supabase mirror (PostgreSQL edition ของ 001-017)", () => {
  it("มีไฟล์ครบ 17 ชื่อเดียวกับ MIGRATION_FILES", () => {
    const dir = supabaseDir();
    expect(MIGRATION_FILES).toHaveLength(17);
  ...
  it("seed/upsert ใช้ ON CONFLICT (003/010/014/016/017) และ 016 ไม่มี IF()", () => {
    const dir = supabaseDir();
    for (const name of [
